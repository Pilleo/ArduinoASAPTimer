...
debounce a button, it will still be at least ms waiting, also cpu will have to compare two not zero values, which is a bit slower.
...
void loop() {
      if (--counter == 0) { //comparing with 0 should be a bit faster and easier for Arduino to process. Correct me if i am wrong. 
        ...
        digitalWrite(13, onOff); // the task itself. 
        }
}
